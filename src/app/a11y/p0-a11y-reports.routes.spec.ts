...
describe('CT-A11Y-RPT — Hardening accesibilidad rutas de reportes', () => {
  ...
  describe('CT-A11Y-RPT-AGE — Distribución por edad', () => {
    ...
    it('expone h1 enfocable, fieldset+legend, aria-required y aria-busy idle', () => {
      ...
      expect(compiled.querySelector('button[type="submit"]')?.getAttribute('aria-busy')).toBe(
        'false',
      );
    });
    ...
    it('success expone role=status y tabla con caption oculto + th scope=col', () => {
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      expect(compiled.querySelector('.age-context[role="status"]')).toBeTruthy();
    ...
    it('422 as_of_date_invalid expone role=alert y conserva filtros', () => {
      ...
      component.onSubmit();
      http.expectOne((r) => r.url === ageUrl).flush(apiProblemAsOfDateInvalidFixture, {
        status: 422,
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      expect(compiled.querySelector('[data-testid="age-error"][role="alert"]')).toBeTruthy();
  ...
  describe('CT-A11Y-RPT-SECTOR — Docentes distintos por sector', () => {
    ...
    it('expone h1 enfocable, fieldset+legend, sin aria-required y aria-busy idle', () => {
      ...
      expect(headings.length).toBe(1);
      expect(headings[0]?.getAttribute('tabindex')).toBe('-1');
      expect(headings[0]?.textContent?.trim()).toBe('Docentes distintos por sector');
    ...
    it('success expone role=status y tabla con caption oculto + th scope=col', () => {
      ...
      http.expectOne((r) => r.url === sectorUrl).flush(teacherCountsBySectorFixture);
      fixture.detectChanges();

      const compiled = fixture.nativeElement as HTMLElement;
      expect(compiled.querySelector('.sector-context[role="status"]')).toBeTruthy();
      expect(compiled.querySelector('[role="alert"]')).toBeNull();
      expect(compiled.querySelector('caption.visually-hidden')).toBeTruthy();
    ...
    it('422 period_invalid expone role=alert y conserva filtros', () => {
      ...
      component.onSubmit();
      http.expectOne((r) => r.url === sectorUrl).flush(apiProblemPeriodInvalidFixture, {
        status: 422,
        statusText: 'Unprocessable Entity',
        headers: new HttpHeaders({ 'Content-Type': 'application/problem+json' }),
      });
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      expect(compiled.querySelector('[data-testid="sector-error"][role="alert"]')).toBeTruthy();
      expect(component.form.controls.periodEnd.value).toBe('2026-07-01');
      expect(compiled.querySelector('[data-testid="sector-results"]')).toBeNull();
    });
  ...
  describe('CT-A11Y-RPT-TOP — Escuelas líderes', () => {
    ...
    it('expone h1 enfocable, fieldset+legend, aria-required y aria-busy idle', () => {
      ...
      expect(headings.length).toBe(1);
      expect(headings[0]?.getAttribute('tabindex')).toBe('-1');
      expect(headings[0]?.textContent?.trim()).toBe('Escuelas líderes por matrícula');
      expect(compiled.querySelector('fieldset legend')).toBeTruthy();
      expect(compiled.querySelectorAll('[aria-required="true"]').length).toBe(1);
      expect(compiled.querySelector('button[type="submit"]')?.getAttribute('aria-busy')).toBe(
        'false',
    ...
    it('success expone role=status y tabla con caption oculto + th scope=col', () => {
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      expect(compiled.querySelector('.top-context[role="status"]')).toBeTruthy();
      expect(compiled.querySelector('[role="alert"]')).toBeNull();
      expect(compiled.querySelector('caption.visually-hidden')).toBeTruthy();
      expect(compiled.querySelectorAll('th[scope="col"]').length).toBe(3);
    });
    ...
    it('200 [] expone empty role=status con botón Reintentar', () => {
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      const empty = compiled.querySelector('[data-testid="top-empty"][role="status"]');
      expect(empty).toBeTruthy();
      expect(empty?.querySelector('button')?.textContent?.trim()).toBe('Reintentar');
      expect(compiled.querySelector('[role="alert"]')).toBeNull();
    });
    ...
    it('404 expone role=alert y conserva filtros', () => {
      ...
      component.onSubmit();
      http.expectOne((r) => r.url === topUrl).flush(apiProblemNotFoundFixture, {
        status: 404,
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      expect(compiled.querySelector('[data-testid="top-error"][role="alert"]')).toBeTruthy();
