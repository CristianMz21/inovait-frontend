...
@Component({
  ...
  template: `
    <section class="route-placeholder" aria-labelledby="route-title">
      ...
      <p role="status">
        Esta sección pertenece a P1 y permanece bloqueada hasta que la puerta P0
        esté aprobada. Las decisiones y la habilitación se registrarán en
...
export class P1LockedComponent {
  ...
  private readonly feature = toSignal(
    this.route.data.pipe(
      map((data) => (data["lockedFeature"] as string) ?? ""),
    ),
    { initialValue: "" },
  );
