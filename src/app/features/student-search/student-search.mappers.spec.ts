...
describe('studentSearchFiltersToParams', () => {
  it('devuelve null cuando falta algún filtro obligatorio', () => {
    expect(studentSearchFiltersToParams({ ...completeFilters, schoolId: null })).toBeNull();
    expect(studentSearchFiltersToParams({ ...completeFilters, gradeId: null })).toBeNull();
    expect(
      studentSearchFiltersToParams({ ...completeFilters, academicYearId: null }),
    ).toBeNull();
  });
  ...
  it('emite asOfDate cuando la operadora lo define y recorta espacios', () => {
    expect(
      studentSearchFiltersToParams({ ...completeFilters, asOfDate: ' 2026-07-10 ' }),
    ).toEqual({
      schoolId: 1,
      gradeId: 1,
      academicYearId: 2,
