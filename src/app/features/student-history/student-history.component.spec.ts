...
describe('StudentHistoryComponent (CT-HIST-COMP)', () => {
  ...
  it('expone h1 enfocable y el botón disabled hasta completar la identidad', () => {
    ...
    expect(headings[0]?.textContent?.trim()).toBe('Historial académico-docente');
  ...
  it('success con 2 años preserva el orden estable (desc por academicYear.startDate)', () => {
    ...
    component.onSubmit();
    fixture.detectChanges();
    http.expectOne((r) => r.url === baseUrl).flush(studentHistorySecondYearFixture);
    fixture.detectChanges();

    const compiled = fixture.nativeElement as HTMLElement;
    const items = compiled.querySelectorAll('li.history-entry');
    expect(items.length).toBe(2);
    const headings = Array.from(compiled.querySelectorAll('.history-card-header h3'));
  ...
  it('200 enrollments [] mapea a empty/noResults con botón Reintentar', () => {
    ...
    const compiled = fixture.nativeElement as HTMLElement;
    const empty = compiled.querySelector('[data-testid="history-empty"][role="status"]');
  ...
  it('404 student_not_found expone role=alert y conserva filtros', () => {
    ...
    component.onSubmit();
    fixture.detectChanges();
    http.expectOne((r) => r.url === baseUrl).flush(apiProblemStudentNotFoundFixture, {
      status: 404,
    ...
    const compiled = fixture.nativeElement as HTMLElement;
    expect(compiled.querySelector('[data-testid="history-error"][role="alert"]')).toBeTruthy();
    expect(component.form.controls.documentNumber.value).toBe('99.001.101');
    expect(compiled.querySelector('[data-testid="history-results"]')).toBeNull();
  });
  ...
  it('400 invalid_request expone role=alert y conserva filtros', () => {
    ...
    component.onSubmit();
    fixture.detectChanges();
    http.expectOne((r) => r.url === baseUrl).flush(apiProblemHistoryBadRequestFixture, {
      status: 400,
